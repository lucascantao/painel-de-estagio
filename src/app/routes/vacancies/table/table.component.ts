...
export class VacanciesTableComponent {
  ...
  onCheckAll() {
    this.isAllSelected() ? this.checkedItems = [] : this.checkedItems = this.vacancies.map(request => request.id);
  }

  isAllSelected() {
    return this.checkedItems.length === this.vacancies.length && this.checkedItems.length > 0;
  }
